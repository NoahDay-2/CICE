...
! that filenames end with 'yyyy.dat' or 'yyyy.r' or 'yyyy.nc'.
      ...
      elseif (index(trim(atm_data_type),'JRA55') > 0) then ! netcdf
         ...
         tmpname = data_file
         write(data_file,'(a,i4.4,a)') tmpname(1:i), yr, '.nc'
      elseif (index(trim(ocn_data_type),'access') > 0) then ! netcdf
         i = index(data_file,'.nc') - 5
         tmpname = data_file
         write(data_file,'(a,i4.4,a)') tmpname(1:i), yr, '.nc'
...
! authors: Noah Day, UAdl (adapted from ocn_data_ncar_init)
      ...
      character(char_len) :: &
        vname(4) ! variable names to search for in file
      data vname /  &
           'sst',      'sss',     'u',     'v' /
      ...
      if (trim(ocn_data_format) == 'nc') then
       ...
       ! Read in ocean forcing data for all 12 months
         do n=1,4    ! field loop start
            ...
            enddo       ! month loop end
         enddo          ! field loop end
         ...
         if (debug_forcing) then
            ...
            vmin = global_minval(ocn_frc_m(:,:,:,1,1),distrb_info,tmask)
            vmax = global_maxval(ocn_frc_m(:,:,:,1,1),distrb_info,tmask)
            if (my_task.eq.master_task)  &
               write (nu_diag,*) 'sst',vmin,vmax

            vmin = global_minval(ocn_frc_m(:,:,:,2,1),distrb_info,tmask)
            vmax = global_maxval(ocn_frc_m(:,:,:,2,1),distrb_info,tmask)
            if (my_task.eq.master_task)  &
               write (nu_diag,*) 'sss',vmin,vmax

            vmin = global_minval(ocn_frc_m(:,:,:,3,1),distrb_info,umask)
            vmax = global_maxval(ocn_frc_m(:,:,:,3,1),distrb_info,umask)
            if (my_task.eq.master_task)  &
               write (nu_diag,*) 'uocn',vmin,vmax

            vmin = global_minval(ocn_frc_m(:,:,:,4,1),distrb_info,umask)
            vmax = global_maxval(ocn_frc_m(:,:,:,4,1),distrb_info,umask)
            if (my_task.eq.master_task)  &
               write (nu_diag,*) 'vocn',vmin,vmax
            ...
            vmin = global_minval(ocn_frc_m(:,:,:,1,2),distrb_info,tmask)
            vmax = global_maxval(ocn_frc_m(:,:,:,1,2),distrb_info,tmask)
            if (my_task.eq.master_task)  &
               write (nu_diag,*) 'sst',vmin,vmax

            vmin = global_minval(ocn_frc_m(:,:,:,2,2),distrb_info,tmask)
            vmax = global_maxval(ocn_frc_m(:,:,:,2,2),distrb_info,tmask)
            if (my_task.eq.master_task)  &
               write (nu_diag,*) 'sss',vmin,vmax

            vmin = global_minval(ocn_frc_m(:,:,:,3,2),distrb_info,umask)
            vmax = global_maxval(ocn_frc_m(:,:,:,3,2),distrb_info,umask)
            if (my_task.eq.master_task)  &
               write (nu_diag,*) 'uocn',vmin,vmax

            vmin = global_minval(ocn_frc_m(:,:,:,4,2),distrb_info,umask)
            vmax = global_maxval(ocn_frc_m(:,:,:,4,2),distrb_info,umask)
            if (my_task.eq.master_task)  &
               write (nu_diag,*) 'vocn',vmin,vmax
...
! authors: Noah Day, UAdl (adapted from ocn_data_ncar)
...
!      midmonth = fix(p5 * real(daymo(mmonth),kind=dbl_kind))  ! exact middle
      ...
      do n = 4, 1, -1
         ...
         if (ixm /= -99) then  ! first half of month
            do i = 1, nx_block
               do j = 1, ny_block
                  sst_data(i,j,1,iblk) = ocn_frc_m(i,j,iblk,n,ixm)
                  sst_data(i,j,2,iblk) = ocn_frc_m(i,j,iblk,n,mmonth)
               enddo
            enddo
         else                 ! second half of month
            sst_data(:,:,1,iblk) = ocn_frc_m(:,:,iblk,n,mmonth)
...
!=======================================================================
!   ROUTINE: wave_spec_data_fixed
!
!   DESCRIPTION: Read in wave forcing given fixed boundary conditions at the ice edge
!
!   Noah Day, University of Adelaide, 2025
!
      subroutine wave_spec_data_fixed
...
!
      ...
      real(kind=dbl_kind), intent(in) :: &
         conc  ! ice concentration in the source cell
      ...
      real(kind=dbl_kind) :: & 
         exp_atten, & ! exponential attenuation factor
         dist         ! distance between cells (m)
      ...
      if (i /= src_i .and. j /= src_j) then
         ! Diagonal neighbor (e.g., NW, SE, etc.)
         dist = sqrt(HTE(src_i, src_j, src_iblk)**2 + HTN(i, j, iblk)**2)
      else if (i /= src_i) then
         dist = HTE(src_i, src_j, src_iblk)
      else if (j /= src_j) then
         dist = HTN(i, j, iblk)
      else
         dist = 0.0_dbl_kind ! Shouldn't happen
      end if

      ! Propagate wave energy with exponential attenuation
      do idx_freq = 1, nfreq
      ...
      if (debug_n_d .and. my_task == master_task) then 
         write(nu_diag,*) subname, '   PROPAGATING WAVES'
         write(nu_diag,*) subname, '   wave direction', dir_code
         write(nu_diag,*) subname, '   i, j, iblk', i, j, iblk
         write(nu_diag,*) subname, '   wave_spectrum(i,j,:,iblk)', &
            wave_spectrum(i, j, :, iblk), ' (m^2/s)'
         write(nu_diag,*) subname, '   dist', dist, '(m)'
         write(nu_diag,*) subname, '   conc', conc
         write(nu_diag,*) subname, '   LAT', tlat(i, j, iblk) * (180.0d0 / acos(-1.0d0)), ' LON', tlon(i, j, iblk) * (180.0d0 / acos(-1.0d0))
...
!
!   DESCRIPTION: Generate Bretschneider wave spectrum
!
!   Noah Day, University of Adelaide, 2025

function SDF_Bretschneider(f, moment_no, Hs, Tp)
   
   integer, intent(in)       :: moment_no
   real(kind=8), intent(in)  :: f       ! frequency (1/s)
   real(kind=8), intent(in)  :: Hs      ! significant wave height (m)
   real(kind=8), intent(in)  :: Tp      ! peak period in seconds (s)
   real(kind=8)              :: SDF     ! spectral density [m²/Hz]

   real(kind=8)              :: fp, f1, f2, f3
   ...
   fp  = 1d0 / Tp     ! peak frequency
   f1  = (5d0/16d0) * Hs**2 * fp**4
   f2  = f**(moment_no - 5)
   f3  = EXP(-1.25d0 * (fp / f)**4)
   SDF_Bretschneider = f1 * f2 * f3
      
end function SDF_Bretschneider

!=======================================================================
!   FUNCTION: fn_Attn_MBK
!
!   DESCRIPTION: Get wave-ice attenuation rate from 
!                Meylan, Bennetts, Kohout, GRL (2014)
!                DOI: 10.1002/2014GL060809
!
!   Noah Day, University of Adelaide, 2025
!
function fn_Attn_MBK(dum_freq)
!
! !USES:
!
! INPUT PARAMETERS:
!
   real(kind=8), intent (in) :: dum_freq  ! freq (1/s)
...
!
   ...
   !   beta1 = 2.947870279251530E-005 ! a(T) = a(w)*(2 pi/T)^n
